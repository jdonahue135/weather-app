...
class App extends React.Component {
  ...
  fetchImage() {
    ...
    let url =
      "https://api.giphy.com/v1/gifs/translate?api_key=wqsy7csWQKiCwyrmlWrbs4KEj1AqqyMc&s=" +
      weather;
    fetch(url, { mode: "cors" })
  ...
  handleFormSubmit(e) {
    e.preventDefault();
    //Validate input
    fetch(getApiUrl(this.state.formInput), { mode: "cors" }).then((resp) => {
      if (resp.ok) {
  ...
  render() {
    return (
      <div>
        <h1>Weather App</h1>
        <Image weather={this.state.weather} gif={this.state.gif} />
        <Display
